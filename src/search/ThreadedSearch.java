...
public class ThreadedSearch<T> implements Runnable {
  ...
  */
  public boolean parSearch(int numThreads, T target, ArrayList<T> list) throws InterruptedException {
	...
	/* Then construct `numThreads` instances of this class (`ThreadedSearch`)
    	...
    	*/
    	...
    	int sizeSect = list.size()/numThreads;
    	for (int i = 0; i < numThreads; i++){
		int startIndex = (i * sizeSect);
		int endIndex = startIndex + sizeSect;
    		threads[i] = new Thread (new ThreadedSearch(target, list, startIndex, endIndex, answer));
		threads[i].start();
    	}
  ...
  //Linear search for each thread, once found the target, set answer to true and break
  public void run() {
